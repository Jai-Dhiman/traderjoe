...
impl VectorStore {
    ...
    pub async fn new(pool: PgPool) -> Result<Self> {
        ...
        sqlx::query("SELECT 1 FROM pg_extension WHERE extname = 'vector'")
            ...
            .ok_or_else(|| anyhow::anyhow!("pgvector extension not found. Please run migrations."))?;
    ...
    pub async fn similarity_search(
        ...
        for row in rows {
            let entry = ContextEntry {
                ...
                similarity: Some(row.get("similarity")),
            };
            results.push(entry);
        }
    ...
    pub async fn context_stats(&self) -> Result<(usize, usize, usize)> {
        ...
        Ok((total as usize, with_embeddings as usize, with_outcomes as usize))
    }
...
mod tests {
    ...
    async fn test_vector_operations() {
        ...
        let vector_store = VectorStore::new(pool).await.expect("Failed to create VectorStore");
        
        // Ensure HNSW index
        vector_store.ensure_hnsw_index("ace_contexts", "embedding")
            .await
        ...
        let embedding = embedder.embed("bullish market sentiment")
            .await
        ...
        vector_store.upsert_context_embedding(
            context_id,
            chrono::Utc::now(),
            market_state,
            decision,
            ...
            None,
            embedding.clone(),
        ).await.expect("Failed to insert context");
        
        // Test similarity search
        let similar_embedding = embedder.embed("positive market outlook")
            .await
            .expect("Failed to generate similar embedding");
        
        let results = vector_store.similarity_search(similar_embedding, 5)
            .await
        ...
        // Test context stats
        let (total, with_embeddings, _) = vector_store.context_stats()
            .await
