...
impl ContextDAO {
    ...
    pub async fn get_recent_contexts(&self, limit: usize) -> Result<Vec<AceContext>> {
        let rows = sqlx::query_as!(
            ...
            SELECT 
                ...
                confidence,
                outcome,
                NULL::vector as "embedding: Option<Vec<f32>>",
                ingested_at
            FROM ace_contexts 
        ...
        info!("Retrieved {} recent contexts", rows.len());
        Ok(rows)
    }
    ...
    pub async fn update_outcome(&self, context_id: Uuid, outcome: &serde_json::Value) -> Result<()> {
    ...
    pub async fn delete_contexts_older_than(&self, cutoff_date: DateTime<Utc>) -> Result<u64> {
        let result = sqlx::query!(
            "DELETE FROM ace_contexts WHERE timestamp < $1",
            cutoff_date
        )
        .execute(&self.pool)
        .await?;
        ...
        info!("Deleted {} contexts older than {}", deleted_count, cutoff_date);
        Ok(deleted_count)
    }
}
...
mod tests {
    ...
    async fn test_insert_and_retrieve_context() {
        ...
        let context_id = dao.insert_context(
            &market_state,
            &decision,
            "Test reasoning",
            0.8,
        ...
        let retrieved = dao.get_context_by_id(context_id)
            .await
        ...
        let recent = dao.get_recent_contexts(1).await.expect("Failed to get recent contexts");
    ...
    async fn test_context_stats() {
        ...
        let stats = dao.get_context_stats()
            .await
